...
class Session {
    ...
    transformUserData(rows) {
        ...
        // 2. Creamos la estructura base omitiendo profile_id y profile_na de la raíz
        const formattedUser = {
            user_id: firstRow.user_id,
            user_na: firstRow.user_na,
            user_pw: firstRow.user_pw,
            person_id: firstRow.person_id,
            person_first_na: firstRow.person_first_na,
            person_last_na: firstRow.person_last_na,
            person_ci: firstRow.person_ci,
            status_id: firstRow.status_id,
            status_na: firstRow.status_na,
        ...
        rows.forEach(row => {
            if (row.profile_id && row.profile_na) {
